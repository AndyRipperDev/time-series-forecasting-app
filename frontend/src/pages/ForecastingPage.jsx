    ...
    <div>
      {loading ? (
        ...
        <div className="my-2 md:my-4 mx-4 md:mx-10">
          <h1 className="text-2xl font-bold md:text-3xl mt-6 mb-4">Forecasts</h1>
          {forecastingResults && (
            <div>
              {Object.keys(forecastingResults.forecasting).map(
                (key, index, { length }) => {
                  return (
                    <div key={key} className="collapse collapse-arrow border border-base-100 bg-base-100 rounded-box">
                      <input type="checkbox" defaultChecked={true} />
                      <div className="collapse-title text-2xl md:text-3xl font-bold bg-base-100">
                        <div className={'flex justify-between items-center'}>
                          <div>{forecastingResults.forecasting[key][Object.keys(forecastingResults.forecasting[key])[0]][0].datasetcolumns.datasets.project.title}</div>
                          <div className="badge badge-lg font-normal bg-base-200 border-base-200 text-base-content">Project</div>
                        </div>
                      </div>
                      <div className="collapse-content">
                        {Object.keys(forecastingResults.forecasting[key]).map(
                          (key2, index2, { length2 }) => {
                            return (
                              <div key={key2} className="collapse collapse-arrow border border-base-100 bg-base-100 rounded-box">
                                <input type="checkbox" defaultChecked={true} />
                                <div className="collapse-title text-xl font-medium ">
                                  <div className={'flex justify-between items-center'}>
                                    <div>{forecastingResults.forecasting[key][key2][0].datasetcolumns.name}</div>
                                    <div className="badge badge-lg font-normal bg-base-200 border-base-200 text-base-content">Column</div>
                                  </div>
                                </div>
                                <div className="collapse-content">
