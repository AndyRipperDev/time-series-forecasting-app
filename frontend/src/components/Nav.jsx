...
import { authAtom, navBigViewAtom, toggleNavBigViewAtom, themeAtom } from 'state';
...
function Nav() {
  ...
  return (
    <nav className={`menu p-3 rounded-r-2xl h-screen flex flex-col justify-between transition-width transition-slowest duration-500 ease-in-out ${navBigView ? "w-72" : "w-20"}`}>
      <ul>
        <li className="mb-16 place-content-end">
          <span onClick={handleNavChange}>
            <label id="navToggle" className={`swap ${navBigView ? "swap-active" : ""}`}>
              <svg className="fill-current w-6 h-6 swap-on"  stroke="currentColor" viewBox="0 0 24 24"
                   xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
              </svg>
              <svg className="fill-current w-6 h-6 swap-off" stroke="currentColor" viewBox="0 0 24 24"
                   xmlns="http://www.w3.org/2000/svg">
      ...
      <ul>
        <li className="mb-2">
          <ThemeToggle className="place-content-start">{navBigView && <NavItemTitle className="capitalize" number={4}>{theme} Theme</NavItemTitle>}</ThemeToggle>
        </li>
        <li className="mb-2">
          <NavLink to="/settings">
